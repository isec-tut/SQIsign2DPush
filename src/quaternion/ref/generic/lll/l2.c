...
/* Parameters used by the SQIsign v2.0 L2 implementation. */
...
static void
quat_lattice_gram_from_q(ibz_mat_4x4_t *G, const quat_lattice_t *lattice, const ibz_t *q)
{
    ibz_t tmp;
    ibz_init(&tmp);
    ...
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j <= i; j++) {
            ...
            /* The trace bilinear form is twice the reduced norm form. */
...
int
quat_lattice_lll(ibz_mat_4x4_t *red,
                 const quat_lattice_t *lattice,
                 const ibz_t *q,
                 int precision)
{
    /* DPE has an extended exponent and does not use an mpf precision. */
    (void)precision;

    ibz_mat_4x4_t G;
    ibz_mat_4x4_init(&G);
    quat_lattice_gram_from_q(&G, lattice, q);
