...
static void random_pair_of_equivalent_ideals(quat_left_ideal_t *I,
                                             quat_left_ideal_t *J) {
  ibz_t normIJ;
  ibz_init(&normIJ);
  ibz_pow(&normIJ, &ibz_const_two,
          TORSION_PLUS_EVEN_POWER -
              (rand() % 5)); // arbitrarily divide out some powers of 2
  (&normIJ, &normIJ, &TORSION_ODD);

  // arbitrarily divide out one of the prime powers
  {
    ibz_t rem;
    ibz_init(&rem);
    ibz_div(&normIJ, &rem, &normIJ,
            &TORSION_ODD_PRIMEPOWERS[rand() % sizeof(TORSION_ODD_PRIMEPOWERS) /
                                     sizeof(*TORSION_ODD_PRIMEPOWERS)]);
  ...
  quat_lideal_create_from_primitive(I, &genI, &normI, &MAXORD_O0,
                                    &QUATALG_PINFTY);
  quat_lideal_create_from_primitive(J, &genJ, &normJ, &MAXORD_O0,
                                    &QUATALG_PINFTY);
...
int _id2iso_test_id2ker_even() {
  ...
  ec_isog_odd_t isog2;
  {
    ...
    id2iso_ideal_to_isogeny_odd(&isog2, &CURVE_E0, &BASIS_ODD_PLUS,
                                &BASIS_ODD_MINUS, &ideal2);

    // check orders
    {
      if (!ibz_is_one(&ideal1.norm) && fp2_is_zero(&isog1.kernel.z)) {
        printf("norm of ideal1 is >1 but kernel is trivial; something is "
               "wrong!\n");
        res &= 0;
      }
      if (!ibz_is_one(&ideal2.norm) && fp2_is_zero(&isog2.ker_plus.z) &&
          fp2_is_zero(&isog2.ker_minus.z)) {
        printf("norm of ideal2 is >1 but kernel is trivial; something is "
               "wrong!\n");
        res &= 0;
      }
    }
