...
static int local_test_point_order_twof(const ec_point_t *P,
                                       const ec_curve_t *E) {
  ec_point_t test;
  copy_point(&test, P);
...
void id2iso_long_two_isog_init(id2iso_long_two_isog_t *isog,
                               const size_t length) {
  isog->length = length;
...
void id2iso_compressed_long_two_isog_init(
    id2iso_compressed_long_two_isog_t *zip, const size_t length) {
  zip->length = length;
...
void id2iso_compressed_long_two_isog_finalize(
...
/**
 * @brief evaluation of an endomorphism beta of norm dividing T² on the 2^f
 * torsion
 *
 ...
 * the matrix given in output correspond to the action of the endomorphism beta
 * on the 2^f torsion the trace is used to lift the sign ambiguity
 */
void endomorphism_evaluation(
    ibz_mat_2x2_t *action_matrix, const quat_left_ideal_t *ideal_beta_L,
    const quat_left_ideal_t *ideal_beta_R, const ibz_t *trace,
    const quat_alg_t *Bpoo, const ec_curve_t *domain,
    const ec_basis_t *basis_minus, const ec_basis_t *basis_plus,
    const ec_basis_t *two_basis, const ibz_t *ibz_two_f) {

  // var dec
  ec_isog_odd_t isog_beta_L, isog_beta_R;
  ec_basis_t basis_R;
  ec_point_t basis_L[2];
  ec_curve_t codomain_L, codomain_R;
  ec_isom_t isom;
  digit_t x1[NWORDS_ORDER] = {0}, x2[NWORDS_ORDER] = {0},
          x3[NWORDS_ORDER] = {0}, x4[NWORDS_ORDER] = {0};
  ...
  id2iso_ideal_to_isogeny_odd(&isog_beta_L, domain, basis_plus, basis_minus,
                              ideal_beta_L);
  id2iso_ideal_to_isogeny_odd(&isog_beta_R, domain, basis_plus, basis_minus,
                              ideal_beta_R);
  ...
  codomain_L = *domain;
#ifndef NDEBUG
  assert(local_test_point_order_twof(&basis_R.Q, &codomain_R));
  assert(local_test_point_order_twof(&basis_R.PmQ, &codomain_R));
  assert(local_test_point_order_twof(&basis_R.P, &codomain_R));
  assert(local_test_point_order_twof(&basis_L[0], &codomain_L));
#endif
...
#ifndef NDEBUG
  assert(local_test_point_order_twof(&basis_L[1], &codomain_R));
  assert(local_test_point_order_twof(&basis_R.P, &codomain_R));
  assert(local_test_point_order_twof(&basis_R.Q, &codomain_R));
  assert(local_test_point_order_twof(&basis_R.PmQ, &codomain_R));
  ...
  assert(ec_is_on_curve(&codomain_R, &basis_L[1]));
#endif
  ...
  // first DLP
#ifndef NDEBUG
  assert(local_test_point_order_twof(&basis_L[0], &codomain_R));
#endif
...
/**
 * @brief Translating an ideal of norm a big power of 2 into the corresponding
 * isogeny
 *
 * @param isog_zip Output : compression of the output isogeny
 * @param basis_minus : odd torsion basis (in the end, this will be the basis
 * pushed through the output isogeny)
 * @param basis_plus : odd torsion basis (in the end, this will be the basis
 * pushed through the output isogeny)
 * @param domain : the starting curve (in the end, this will be the codomain of
 * the output isogeny)
 * @param kernel_dual : the dual of the kernel of the last step of
 * isog_start_two (in the end this will be the kernel of the dual of the last
 * step of the output isogeny)
 * @param gen_input : quaternion element, element of a maximal order O,
 * generator of the O-ideal to be translated
 * @param length : the length of the chain to be translated
 * @param lideal_start_small : a small ideal equivalent to lideal_start_two of
 * right order equal to O
 * @param lideal_start_two : O0-ideal of norm a power of 2 equivalent to
 * lideal_start_small, corresponding to an isogeny isog_start_two
 ...
 * The ideal to be translated is equal to O < gen_input, 2^e> where O =
 * OR(lideal_start_small)
 *
 * assumes that the ideal given in input has exactly norm 2^e where e = length *
 * f (where f = TORSION_PLUS_EVEN_POWER) when used for compressing an isogeny,
 * we assume that the curve given in input is normalized!!
 */
int id2iso_ideal_to_isogeny_two_long_power_of_2(
    id2iso_compressed_long_two_isog_t *isog_zip, ec_curve_t *domain,
    ec_basis_t *basis_minus, ec_basis_t *basis_plus, ec_point_t *kernel_dual,
    const quat_alg_elem_t *gen_input, const int length,
    const quat_left_ideal_t *lideal_start_small,
    const quat_left_ideal_t *lideal_start_two, const quat_alg_elem_t *gen_two,
  ...
  quat_left_ideal_t lideal_equiv;
  quat_left_ideal_init(&lideal_equiv);
  quat_order_t right_order;
  quat_order_init(
      &right_order); // TODECIDE we might avoid this by solving solve linear
                     // combination without using the right order
  quat_alg_coord_t coeffs;

  int const f = TORSION_PLUS_EVEN_POWER;
  ibz_t ibz_two_f, temp, remainder;
  ec_basis_t temp_odd_basis[2];
  ec_curve_t temp_domain;
  ec_point_t temp_kernel_dual;
  ec_basis_t temp_two_basis;
  ec_isog_even_t isog;
  ec_point_t pushed_points[7];
  ibz_mat_2x2_t action_matrix;
  digit_t digit_a[NWORDS_ORDER] = {0};
  digit_t digit_b[NWORDS_ORDER] = {0};
  ibz_t ibz_a, ibz_b;
  ...
  quat_alg_elem_init(&gen_input_temp);
  ...
  // first, we perform all operations over the quaternions to verify that no
  // failure occurs before doing the costly ec operations
  ...
  //  - lideal_small is a small O0-ideal (in the sense that it has somewhat
  //  small norm for ideals of the same class) whose right order is the
  //  endomorphism ring of the domain of the (ind+1)-th isogeny to be translated
  //    in particular at ind = 0, it is simply lideal_start_small
  //  - gen is the generator of an ideal of norm n equivalent to
  //  lideal_start_small. We call it lideal_equiv even though this ideal is
  //  never explicitly computed, and it is equal to lideal_equiv = lideal_small
  //  * gen / norm
  //    (lideal_small) ( = O0 < gen, n(gen)/n(lideal_small) > when ind is bigger
  //    than 0). conj(gen) is contained in lideal_small. At ind = 0, we have
  //    lideal_equiv = lideal_small
  //  - gen_input_temp is an element of the right order of lideal_equiv
  //  generating the ideal of norm 2^f corresponding to the (ind+1)-th isogeny
  //  to be translated.
  //  - gen_two_temp is an element of the right order of lideal_equiv such that
  //  the principal ideal O0 < gen_two_temp > = lideal_two * conj(lideal_equiv)
  //  where lideal_two corresponds to the isogeny of degree 2^*  connecting E0
  //  to the domain of the (ind+1)-th isogeny to be translated

  // At the beginning of iteration ind:
  //  - lideal_small is set to be lideal_equiv times an ideal of norm 2^f
  //  corresponding to the (ind+1)-th isogeny to be translated
  // During iteration ind, all the other values will be updated

  // The main goal of iteration ind is to find an endomorphism beta contained
  // inside the right order of lideal_equiv of step ind+1 (which is isomorphic
  // to the endomorphism ring of the codomain of the (ind+1)-th isogeny to be
  // translated) this endomorphism beta will be used to compute the two ideals
  // ideal_beta_L[ind],ideal_beta_R[ind] and the trace trace_beta[ind] which are
  // required to actually compute the (ind+1)-th isogeny to be translated. Two
  // coefficients linear_comb[ind] will also be necessary, it is also computed
  // in the loop below.
  ...
  // for debug we check that all values are contained in the correct orders and
  // lattices
  assert(quat_lattice_contains(&coeffs, &STANDARD_EXTREMAL_ORDER.order,
                               &gen_two_temp, Bpoo));
  assert(quat_lattice_contains(&coeffs, &lideal_start_two->lattice,
                               &gen_two_temp, Bpoo));
  assert(!quat_lattice_contains(&coeffs, &lideal_start_small->lattice,
                                &gen_two_temp, Bpoo));
  ...
  for (int ind = 0; ind < length; ind++) {
    if (ind > 0) {
      ...
      // if we set lideal_equiv = ideal_small * gen / norm(lideal_small) = O0 <
      // gen, n(gen)/n(lideal_small) > we have set (at the end of the previous
      // iteration) gen_input_temp = conj ( gen ) / norm(gen ) * gen_input_temp
      // * gen and lideal_small = lideal_equiv * O_R(lideal_equiv) <
      // gen_input_temp , 2^f >  = O0 <  make_primitive (gen * gen_input_temp) ,
      // n(gen) * 2^f /n (lideal_small) >
      ...
      assert(quat_lattice_contains(&coeffs, &lideal_equiv.lattice, &quat_temp,
                                   Bpoo));

      // temp = n * 2^f
      ibz_mul(&temp, &n, &ibz_two_f);
      quat_lideal_make_primitive_then_create(&lideal_small, &quat_temp, &temp,
                                             &STANDARD_EXTREMAL_ORDER.order,
                                             Bpoo);
    }
    // copying n in temp
    ibz_copy(&temp, &n);
    ...
    found = klpt_eichler_special_norm(&beta, &n_beta, &gen, &n, &lideal_small,
                                      &gen_constraint, Bpoo);
    if (!found) {
      break;
    }
    ...
    // this includes computation of an updated values of gen_two_temp and
    // gen_input_temp, of the ideals ideal_beta_L[ind] and ideal_beta_R[ind], of
    // the trace of beta as beta_trace[ind], and of the linear_combination
    // linear_comb[ind]
    ...
    if (ind > 0) {
      // we can adjust the scalar of gen_input_temp, because it was adjusted for
      // gen_constraint during the execution of eichler norm fixed
    ...
    quat_alg_conj(&quat_temp, &gen);
    assert(quat_lattice_contains(&coeffs, &lideal_small.lattice, &quat_temp,
                                 Bpoo));
    ...
    // for debug we check that our updated element are contained in the correct
    // orders
    ...
    // for debug we check that gen_two_temp is indeed contained inside
    // order.order
    assert(quat_lattice_contains(&coeffs, &STANDARD_EXTREMAL_ORDER.order,
                                 &gen_two_temp, Bpoo));
    ...
    // ideal_beta_L = O0 < beta * gen_two_temp, N>, ideal_beta_R = O0 <
    // conj(beta) * gen_two_temp, N'> with N N' = n_beta
    ...
    quat_lideal_make_primitive_then_create(
        &ideal_beta_L[ind], &quat_temp, &temp, &STANDARD_EXTREMAL_ORDER.order,
        Bpoo);
    ...
    quat_lideal_make_primitive_then_create(
        &ideal_beta_R[ind], &quat_temp, &temp, &STANDARD_EXTREMAL_ORDER.order,
        Bpoo);
    ...
    // TODUPDATE : it is only needed if trace is even so we may check that to
    // avoid doing that operation every time
    quat_alg_make_primitive(&coeffs, &temp, &gen_input_temp, &right_order,
                            Bpoo);
    ibz_mul(&gen_input_temp.denom, &gen_input_temp.denom, &temp);
    quat_alg_normalize(&gen_input_temp);

    // computation of the linear_combination such that linear_comb[ind][0] +
    // beta * linear_comb[ind][1] sends the kernel of the dual of the 2^f
    // isogeny of step ind-1 to the kernel of the 2^f isogeny of step ind
    found = klpt_find_linear_comb(&linear_comb[ind], &beta, &right_order,
                                  &ibz_two_f, f, &gen_constraint,
                                  &gen_input_temp, Bpoo);
    if (!found) {
      break;
      // TODEBUG
    ...
    lideal_generator_ok = quat_lideal_generator_coprime(
        &gen, &lideal_equiv, &ibz_const_two, Bpoo, 0);
  ...
  // the quaternion computation has succeeded, we can proceed to perform all
  // elliptic curve and isogeny operations
  if (found) {
    ...
    for (int ind = 0; ind < length; ind++) {
      ...
      {
        // normalize the curve so that deterministic bases are independent of
        // formulas
        ec_isom_t isom;
        ec_curve_normalize(&temp_domain, &isom, &temp_domain);
        for (int i = 0; i < 2; ++i) {
          ec_iso_eval(&temp_odd_basis[i].P, &isom);
          ec_iso_eval(&temp_odd_basis[i].Q, &isom);
          ec_iso_eval(&temp_odd_basis[i].PmQ, &isom);
        }
        ec_iso_eval(&temp_kernel_dual, &isom);
      }
      ...
      assert(local_test_point_order_twof(&temp_two_basis.P, &temp_domain));
      assert(local_test_point_order_twof(&temp_two_basis.Q, &temp_domain));
      assert(local_test_point_order_twof(&temp_two_basis.PmQ, &temp_domain));
#endif

      // for each step, we must evaluate the endomorphism, compute the next
      // isogny, push the torsion and compute the compression

      // evaluation of the endomorphism
      endomorphism_evaluation(&action_matrix, &ideal_beta_L[ind],
                              &ideal_beta_R[ind], &beta_trace[ind], Bpoo,
                              &temp_domain, &temp_odd_basis[0],
                              &temp_odd_basis[1], &temp_two_basis, &ibz_two_f);

      // computation of the linear combination of points to get the kernel
      // linear_comb[ind][0] = linear_comb[ind][1] * action_matrix[0][0] +
      // linear_comb[ind][0]
      ...
      // in that case we can rescale the basis and compute the zipped value
      // directly
      if (ind > 0) {
        assert(0 != ibz_get(&linear_comb[ind][1]) % 2);
        ibz_invmod(&linear_comb[ind][1], &linear_comb[ind][1], &ibz_two_f);
        ibz_mul(&linear_comb[ind][0], &linear_comb[ind][0],
                &linear_comb[ind][1]);
        ibz_mod(&linear_comb[ind][0], &linear_comb[ind][0], &ibz_two_f);
      ...
      ec_biscalar_mul(&isog.kernel, &temp_domain, digit_a, digit_b,
                      &temp_two_basis);
#ifndef NDEBUG
      assert(local_test_point_order_twof(&isog.kernel, &temp_domain));
#endif
      ...
      if (ind == 0) {
        ...
        ec_dlog_2(digit_a, digit_b, &temp_two_basis, &isog.kernel,
                  &temp_domain);
        ...
        // set the bit_first_step as 0 when we can get a generator as
        // temp_two_basis.P + s temp_two_basis.Q if not, then this means we can
        // write as Q + s P
...
#ifndef NDEBUG
      assert(local_test_point_order_twof(&temp_kernel_dual, &temp_domain));
#endif
...
void from_1ijk_to_O0basis(ibz_vec_4_t *vec, const quat_alg_elem_t *el) {
  ...
  ibz_add(&(*vec)[2], &(*vec)[2],
          &(*vec)[2]); // double (not optimal if el->denom is even...)
  ibz_copy(&(*vec)[3],
           &el->coord[3]); // double (not optimal if el->denom is even...)
...
// untested
void matrix_of_endomorphism_even(ibz_mat_2x2_t *mat,
...
void id2iso_ideal_to_kernel_dlogs_even(ibz_vec_2_t *vec,
  ...
  // construct the matrix of the dual of alpha on the 2^f-torsion
  {
    ...
    int lideal_generator_ok;
    lideal_generator_ok =
        quat_lideal_generator(&alpha, lideal, &QUATALG_PINFTY, 0);
    assert(lideal_generator_ok);
...
void id2iso_ideal_to_isogeny_even(ec_isog_even_t *isog,
...
// works in place
void matrix_application_even_basis(ec_basis_t *bas, const ec_curve_t *E,
                                   ibz_mat_2x2_t *mat, int f) {
...
// works in place
void matrix_application_three_basis(ec_basis_t *bas, const ec_curve_t *E,
                                    ibz_mat_2x2_t *mat, int f) {
  digit_t scalars[2][NWORDS_ORDER] = {0};
...
void id2iso_ideal_to_kernel_dlogs_odd(ibz_vec_2_t *vec, ec_degree_odd_t *deg,
                                      const quat_left_ideal_t *lideal) {
  ibz_t tmp;
  ibz_init(&tmp);
  ...
  // construct the matrix of the dual of alpha on the T-torsion
  {
    ...
    assert(lideal_generator_ok);
    assert(
        ibz_divides(&ibz_const_two,
                    &alpha.denom)); // denominator is invertible mod T, ignore
...
/**
 * @brief Translating an ideal of odd norm dividing p²-1 into the corresponding
 * isogeny
 *
 ...
 * the coefficients extracted from the ideal are to be applied to basis_minus
 * and basis_plus to compute the kernel of the isogeny.
 *
 */

void id2iso_ideal_to_isogeny_odd(ec_isog_odd_t *isog, const ec_curve_t *domain,
                                 const ec_basis_t *basis_plus,
                                 const ec_basis_t *basis_minus,
                                 const quat_left_ideal_t *lideal_input) {
  digit_t scalars_plus[2][NWORDS_ORDER] = {0},
          scalars_minus[2][NWORDS_ORDER] = {0};
  {
    ibz_vec_2_t vec;
    ibz_vec_2_init(&vec);
  ...
  ec_biscalar_mul(&isog->ker_minus, domain, scalars_minus[0], scalars_minus[1],
                  basis_minus);
...
void id2iso_kernel_dlogs_to_ideal(quat_left_ideal_t *lideal,
  ...
  ibz_vec_2_t ker;
  ibz_vec_2_init(&ker);
  ibz_crt(&ker[0], &(*vec2)[0], &(*vec3)[0], &TORSION_PLUS_2POWER,
          &TORSION_PLUS_3POWER);
  ibz_crt(&ker[1], &(*vec2)[1], &(*vec3)[1], &TORSION_PLUS_2POWER,
          &TORSION_PLUS_3POWER);
  ...
  quat_lideal_create_from_primitive(lideal, &gen, &TORSION_PLUS_23POWER,
...
void id2iso_kernel_dlogs_to_ideal_three(quat_left_ideal_t *lideal,
                                        const ibz_vec_2_t *vec3) {
  ...
  quat_lideal_create_from_primitive(lideal, &gen, &TORSION_PLUS_3POWER,
                                    &MAXORD_O0, &QUATALG_PINFTY);
...
void change_of_basis_matrix_two(ibz_mat_2x2_t *mat, const ec_basis_t *B1,
                                const ec_basis_t *B2, const ec_curve_t *E) {
  // TODO
  digit_t x1[NWORDS_ORDER] = {0}, x2[NWORDS_ORDER] = {0},
          x3[NWORDS_ORDER] = {0}, x4[NWORDS_ORDER] = {0};
  ...
  // assert(local_test_point_order_twof(&(B1->P), E));
  // assert(local_test_point_order_twof(&(B1->Q), E));
  // assert(local_test_point_order_twof(&(B1->PmQ), E));
#ifndef NDEBUG
  assert(local_test_point_order_twof(&(B2->P), E));
  assert(local_test_point_order_twof(&(B2->Q), E));
  assert(local_test_point_order_twof(&(B2->PmQ), E));
#endif
  // clock_t start = clock();
  // ec_dlog_2(x1, x2, B2, &(B1->P), E);
  // clock_t end = clock();
  // printf("ec_dlog_2 (P): %f ms\n",
  //        (double)(end - start) * 1000 / CLOCKS_PER_SEC);
  // start = clock();
  // ec_dlog_2(x3, x4, B2, &(B1->Q), E);
  // end = clock();
  // printf("ec_dlog_2 (Q): %f ms\n",
  //        (double)(end - start) * 1000 / CLOCKS_PER_SEC);
  ec_dlog_2_tate(x1, x2, x3, x4, B2, B1, E, TORSION_PLUS_EVEN_POWER);
  ...
  ibz_copy_digit_array(&i4, x4);
  // curve_print("E = ", *E);
  // point_print("B2->P = ", B2->P);
  // point_print("B2->Q = ", B2->Q);
  // point_print("B2->PmQ = ", B2->PmQ);
  // point_print("B1->P = ", B1->P);
  // point_print("B1->Q = ", B1->Q);
  // point_print("B1->PmQ = ", B1->PmQ);
  // ibz_printf("x1 = %Zd\n", &i1);
  // ibz_printf("x2 = %Zd\n", &i2);
  // ibz_printf("x3 = %Zd\n", &i3);
  // ibz_printf("x4 = %Zd\n", &i4);
  ...
  // ibz_init(&iA_re);
  // ibz_init(&iA_im);
  // ibz_copy_digit_array(&iA_re, E->A.re);
  // ibz_copy_digit_array(&iA_im, E->A.im);
  // // ibz_printf("DEBUG: E->A = %Zd + i*%Zd\n", &iA_re, &iA_im);
  // ibz_finalize(&iA_re);
  // ibz_finalize(&iA_im);
  ...
  ibz_to_digit_array(x5, &i5);
  ibz_to_digit_array(x6, &i6);
...
// (mat*v).B2 = v.B1
// where "." is the dot product, defined as (v1,v2).(P,Q) = v1*P + v2*Q
void change_of_basis_matrix_three(ibz_mat_2x2_t *mat, const ec_basis_t *B1,
                                  const ec_basis_t *B2, const ec_curve_t *E) {
  // TODO
  digit_t x1[NWORDS_ORDER] = {0}, x2[NWORDS_ORDER] = {0},
          x3[NWORDS_ORDER] = {0}, x4[NWORDS_ORDER] = {0};
  ...
  ec_dlog_3_tate(x1, x2, x3, x4, B2, B1, E, EXPONENT_THREE);
  ...
  // clock_t end = clock();
  // printf("change_of_basis_matrix_three: %f ms\n",
  //        (double)(end - start) * 1000 / CLOCKS_PER_SEC);
