...
static void
jac_init(jac_point_t *P) { // Initialize Montgomery in Jacobian coordinates as
...
static bool
is_jac_equal(const jac_point_t *P,
             const jac_point_t
                 *Q) { // Evaluate if two points in Jacobian coordinates (X:Y:Z)
                       // are equal Returns 1 (true) if P=Q, 0 (false) otherwise
  fp2_t t0, t1, t2, t3;
...
void DBL(jac_point_t *Q, jac_point_t const *P,
         ec_curve_t const *
             AC) { // Doubling on a Montgomery curve, representation in Jacobian
...
static void ec_dlog_2_step(
    digit_t *x, digit_t *y, const jac_point_t *R, const int f, const int B,
    const jac_point_t *Pe2, const jac_point_t *Qe2, const jac_point_t *PQe2,
    const ec_curve_t
        *curve) { // Based on Montgomery formulas using Jacobian coordinates
  int i, j;
  digit_t value = 1;
...
void ec_dlog_2(
  ...
  int i;
  digit_t w0 = 0, z0 = 0, x0 = 0, y0 = 0, x1 = 0, y1 = 0, w1 = 0, z1 = 0, e, e1,
          f, f1, f2, f2div2, f22, w, z;
  digit_t fp2[NWORDS_ORDER] = {0}, xx[NWORDS_ORDER] = {0},
          yy[NWORDS_ORDER] = {0}, ww[NWORDS_ORDER] = {0},
          zz[NWORDS_ORDER] = {0};
  digit_t f22_p[NWORDS_ORDER] = {0}, w_p[NWORDS_ORDER] = {0},
          z_p[NWORDS_ORDER] = {0}, x0_p[NWORDS_ORDER] = {0},
          y0_p[NWORDS_ORDER] = {0}, w0_p[NWORDS_ORDER] = {0},
          z0_p[NWORDS_ORDER] = {0};
  ...
  ec_dlog_2_step(&w0, &z0, &TT, (int)f2, (int)f2div2, &Pe2[0], &Qe2[0],
                 &PQe2[0], &curvenorm);
  ...
  ec_dlog_2_step(&x0, &y0, &R2r0, (int)e1, (int)f2div2, &Pe2[0], &Qe2[0],
                 &PQe2[0], &curvenorm);
  ...
  ec_dlog_2_step(&w1, &z1, &TT, (int)f2, (int)f2div2, &Pe2[0], &Qe2[0],
                 &PQe2[0], &curvenorm);
  ...
  ec_dlog_2_step(&x1, &y1, &R2r1, (int)e1, (int)f2div2, &Pe2[0], &Qe2[0],
                 &PQe2[0], &curvenorm);
  ...
  mp_shiftl_any(xx, (unsigned int)e, NWORDS_ORDER);
  mp_shiftl_any(yy, (unsigned int)e, NWORDS_ORDER);
  mp_add(scalarP, &xx[0], &ww[0], NWORDS_ORDER);
  mp_add(scalarQ, &yy[0], &zz[0], NWORDS_ORDER);

  fp_copy(fp2, TWOpFm1); // 2^(f-1)

  // If scalarP > 2^(f-1) or (scalarQ > 2^(f-1) and (scalarP = 0 or scalarP =
  // 2^(f-1))) then output -scalarP mod 2^f, -scalarQ mod 2^f
...
static void ec_dlog_3_step(
    digit_t *x, digit_t *y, const jac_point_t *R, const int f, const int B,
    const jac_point_t *Pe3, const jac_point_t *Qe3, const jac_point_t *PQe3,
    const ec_curve_t
        *curve) { // Based on Montgomery formulas using Jacobian coordinates
  int i, j;
  digit_t value = 1;
  jac_point_t P, Q, PQp, PQ2p, P2Qp, P2Q2p, P2p, Q2p, TT, SS,
      Te[POWER_OF_3 / 2],
      Re[POWER_OF_3 / 2]; // Storage could be reduced to e points
  jac_point_t PQep0, PQ2ep0, P2Qep0, P2Q2ep0, P2ep0, Q2ep0, PQep1, PQ2ep1,
      P2Qep1, P2Q2ep1, P2ep1, Q2ep1;
...
void ec_dlog_3(
    ...
    const ec_curve_t *curve) { // Optimized implementation based on Montgomery
                               // formulas using Jacobian coordinates
  int i;
  digit_t w0 = 0, z0 = 0, x0 = 0, y0 = 0, x1 = 0, y1 = 0, w1 = 0, z1 = 0, e, f,
          f1, f1div2;
  digit_t fp2[NWORDS_ORDER] = {0}, xx[NWORDS_ORDER] = {0},
          yy[NWORDS_ORDER] = {0}, ww[NWORDS_ORDER] = {0},
          zz[NWORDS_ORDER] = {0};
  ...
  f1div2 = f1 >> 1;
  ec_dlog_3_step(&w0, &z0, &TT, (int)f1, (int)f1div2, &Pe3[0], &Qe3[0],
                 &PQe3[0], &curvenorm);
  ...
  ec_dlog_3_step(&x0, &y0, &R2r0, (int)e, (int)f1div2, &Pe3[0], &Qe3[0],
                 &PQe3[0], &curvenorm);
  ...
  // If scalarP > Floor(3^f/2) or (scalarQ > Floor(3^f/2) and scalarP = 0) then
  // output -scalarP mod 3^f, -scalarQ mod 3^f
