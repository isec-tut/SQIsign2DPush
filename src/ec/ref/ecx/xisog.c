...
void xisog_2(ec_point_t *B, ec_point_t const P) {
  ...
  // ISOG_MEASURE_END(&global_isog_counters.isog2_ops, s);
}
...
void xisog_4(ec_point_t *B, ec_point_t const P) {
  ...
  // ISOG_MEASURE_END(&global_isog_counters.isog4_ops, s);
}
...
void xisog_2_singular(ec_point_t *B24, ec_point_t A24) {
  ...
  // Store constants for evaluation: K[0].x = sX, K[0].z = sZ
  fp2_copy(&K[0].x, &sX);
  fp2_copy(&K[0].z, &sZ);
  ...
  fp2_add(&B24->z, &B24->z, &B24->z);
  // ISOG_MEASURE_END(&global_isog_counters.isog2_sing_ops, s);
}
...
void xisog_4_singular(ec_point_t *B24, ec_point_t const P, ec_point_t A24) {
  ...
  // ISOG_MEASURE_END(&global_isog_counters.isog4_sing_ops, s);
}

void xisog_3(ec_point_t *B24minus, ec_point_t const P) {
  // ISOG_MEASURE_START(s);
  // global_isog_counters.isog3_count++;

  fp2_t r1, r2, r3, r4, r5;

  // K[0].x = X3-Z3, K[1].x = X3+Z3, reused by xeval_3()
  ...
  fp2_mul(&B24minus->x, &r5, &r3); // A24' = R5 * R3
  ...
  fp2_sub(&B24minus->z, &r5, &B24minus->x); // C24' = R5 - A24'

  // ISOG_MEASURE_END(&global_isog_counters.isog3_ops, s);
}
...
void xisog_t(ec_point_t *B, uint64_t const i, ec_point_t const A) {
  // ISOG_MEASURE_START(s);
  // (isog3_count is incremented in xisog wrapper)
  int j;
  ...
  for (j = 1; j < (int)p_plus_minus_bitlength[i]; j++) {
    fp2_sqr(&tmp_a, &tmp_a);
    fp2_sqr(&tmp_d, &tmp_d);
    if ((((int)TORSION_ODD_PRIMES[i] >>
          ((int)p_plus_minus_bitlength[i] - j - 1)) &
         1) != 0) {
...
void xisog_s(ec_point_t *B, uint64_t const i, ec_point_t const A) {
  ...
  // fp2_t EJ_0[sJ][3],	// quadratic factors of one polynomial to be used in a
  // resultant
  //    EJ_1[sJ][3];	// quadratic factors of one polynomial to be used in a
  //    resultant
  ...
  fp2_t A24, A24m, t24,
      t24m; // <---- JORGE creo que podemos omitir estas variables, se usan
            // cuando ya no se requiren los valores de la entrada A (podemos
            // cambiar estos t's por B[0] y B[1]
  ...
  fp2_mul(
      &t24m, &hk_1,
      &r1); // output of algorithm 2 with alpha =-1 and without the demoninator
  ...
  fp2_mul(
      &t24, &hk_0,
      &r0); // output of algorithm 2 with alpha = 1 and without the demoninator
  ...
  // Now, we have d = (A24m / A24) where the image Montgomery cuve coefficient
  // is
  //      B'   2*(1 + d)   2*(A24 + A24m)
  // B = ---- = --------- = --------------
  //      C      (1 - d)     (A24 - A24m)
