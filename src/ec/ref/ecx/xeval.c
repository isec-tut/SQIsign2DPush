...
// global_isog_counters.eval2_count += lenQ;
  fp2_t t0, t1, t2;
...
// ISOG_MEASURE_END(&global_isog_counters.eval2_ops, s);
}

// Degree-2 isogeny evaluation with kernel generated by P = (0, 0)
// This function requires the curve coefficient A24 = (A+2C:4C) of the domain
// curve
void xeval_2_singular(ec_point_t *R, const ec_point_t *Q, const int lenQ) {
// ISOG_MEASURE_START(s);
// global_isog_counters.eval2_sing_count += lenQ;
  fp2_t t0, t1, c;
  // c = 4 * K[0].x / K[0].z
  fp2_copy(&t1, &K[0].z);
  fp2_inv(&t1);
  ...
  for (int i = 0; i < lenQ; i++) {
    fp2_sub(&t0, &Q[i].x, &Q[i].z);
    fp2_sqr(&t1, &t0); // (x-z)^2
    fp2_mul(&t0, &Q[i].x, &Q[i].z);
    fp2_mul(&R[i].z, &t0, &c);
    fp2_copy(&R[i].x, &t1);
  }
// ISOG_MEASURE_END(&global_isog_counters.eval2_sing_ops, s);
}

// Degree-4 isogeny evaluation with kenerl generated by P such that [2]P != (0,
// 0)
void xeval_4(ec_point_t *R, const ec_point_t *Q, const int lenQ) {
// ISOG_MEASURE_START(s);
// global_isog_counters.eval4_count += lenQ;
  fp2_t t0, t1;
...
// ISOG_MEASURE_END(&global_isog_counters.eval4_ops, s);
}

// Degree-4 isogeny evaluation with kenerl generated by P such that [2]P = (0,
// 0) Must call after xisog_4_singular
void xeval_4_singular(ec_point_t *R, const ec_point_t *Q, const int lenQ,
                      const ec_point_t P) {
// ISOG_MEASURE_START(s);
// global_isog_counters.eval4_sing_count += lenQ;
  fp2_t t0, t1, t2;
  for (int i = 0; i < lenQ; i++) {
    fp2_add(&t0, &Q[i].x, &Q[i].z);
...
// ISOG_MEASURE_END(&global_isog_counters.eval4_sing_ops, s);
}

void xeval_3(ec_point_t *R, const ec_point_t *Q, const int lenQ) {
// ISOG_MEASURE_START(s);
// global_isog_counters.eval3_count += lenQ;

  fp2_t r1, r2, r3;
  for (int i = 0; i < lenQ; i++) {
    fp2_add(&r1, &Q[i].x, &Q[i].z);
    fp2_sub(&r2, &Q[i].x, &Q[i].z);
    ...
    fp2_mul(&R[i].x, &r3, &Q[i].x);
    fp2_mul(&R[i].z, &r1, &Q[i].z);
  }

// ISOG_MEASURE_END(&global_isog_counters.eval3_ops, s);
}

// Isogeny evaluation on Montgomery curves
// Recall: K has been computed in Twisted Edwards model and none extra additions
// are required.
...
// ISOG_MEASURE_START(s);
  // =================================================================================
  assert(TORSION_ODD_PRIMES[i] >
         gap);   // Ensuring velusqrt is used for l_i > gap
  ...
  // We require the curve coefficient A = A'/C ... well, a multiple of these
  // ones
  fp2_t Ap;
  ...
  //                   ~~~~~~~~
  //                    |    |
  // Computing E_J(W) = |    | [ F0(W, x([j]P)) * alpha^2 + F1(W, x([j]P)) *
  // alpha + F2(W, x([j]P)) ]
  //                    j in J
  // In order to avoid costly inverse computations in fp, we are gonna work with
  // projective coordinates In particular, for a degree-l isogeny construction,
  // we need alpha = X/Z and alpha = Z/X (i.e., 1/alpha)

  // fp2_t EJ_0[sJ][3]; // EJ_0[j][2] factors of one polynomial to be used in a
  // resultant

  fp2_t XZ_add, XZj_add, XZ_sub, XZj_sub, AXZ2, CXZ2, CX2Z2, t1, t2;
  ...
  for (j = 0; j < sJ; j++) {
    ...
    // C * [ (-2*Xj*Zj)*(alpha^2 + 1) + (-2*alpha)*(Xj^2 + Zj^2)] + [A' *
    // (-2*Xj*Zj) * (2*X*Z)] where alpha = X/Z
    ...
    fp2_sub(&t1, &t2, &t1); // [C * (X^2 + Z^2)] * (-4 * Xj * Zj) - [2 * (Xj^2 +
                            // Zj^2)] * (2 * [ C * (X * Z)])

    fp2_mul(&t2, &AXZ2, &XZJ4[j]); // (2 * [A' * (X * Z)]) * (-4 * Xj * Zj)
    fp2_add(&EJ_0[j][1], &t1,
            &t2); // This is our desired equation but multiplied by 2
    fp2_add(&EJ_0[j][1], &EJ_0[j][1],
            &EJ_0[j][1]); // This is our desired equation but multiplied by 4
  };
  ...
  if (!scaled) {
    ...
    multieval_unscaled(leaves, ptree_EJ[0], 2 * sJ + 1, rtree_hI,
                       (const fp2_t *)rtree_A, ptree_hI, deg_ptree_hI, 0, sI);
  } else {
  ...
  fp2_mul(&t1, &hk_1, &r1); // output of algorithm 2 with 1/alpha = Z/X and
                            // without the demoninator
  fp2_sqr(&t1, &t1);
  fp2_mul(&(Q->x), &t1, &P.x);

  fp2_mul(&t2, &hk_0, &r0); // output of algorithm 2 with alpha = X/Z and
                            // without the demoninator
  fp2_sqr(&t2, &t2);
  fp2_mul(&(Q->z), &t2, &P.z);
}