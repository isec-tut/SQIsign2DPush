...
void ladder3pt(ec_point_t *R, fp_t const m, ec_point_t const *P,
               ec_point_t const *Q, ec_point_t const *PQ, ec_point_t const *A) {
  ec_point_t X0, X1, X2;
  copy_point(&X0, Q);
  copy_point(&X1, P);
  copy_point(&X2, PQ);
...
static void xeval_2_singular_unused(ec_point_t *R, const ec_point_t *Q,
                                    const int lenQ) {
  fp2_t t0, t1;
...
int main(int argc, char *argv[]) {
  ...
  // ----------------- TEST FOR NONZERO 2-ISOGENIES VS
  // 4-ISOGENIES----------------- //
  ...
  for (int iter = 0; iter < TEST_LOOPS; iter++) {
    ...
    printf("[%3d%%] Testing 2^f-isogenies (nonzero)",
           100 * iter / (int)TEST_LOOPS);
    fflush(stdout);
    printf("\r\x1b[K");
  ...
  for (int iter = 0; iter < TEST_LOOPS / 10; iter++) {
    ...
    printf("[%3d%%] Testing odd-degree isogenies",
           100 * iter / ((int)TEST_LOOPS / 10));
    fflush(stdout);
    printf("\r\x1b[K");
    ...
    for (int i = 0; i < P_LEN; i++) {
      ...
      while (j < TORSION_ODD_POWERS[i]) {
        xMULv2(&isog.ker_plus, &isog.ker_plus, &TORSION_ODD_PRIMES[i],
               p_plus_minus_bitlength[i], &A24);
        j++;
      }
    }
    for (int i = P_LEN; i < P_LEN + M_LEN; i++) {
      ...
      int j = isog.degree[i];
      while (j < TORSION_ODD_POWERS[i]) {
        xMULv2(&isog.ker_minus, &isog.ker_minus, &TORSION_ODD_PRIMES[i],
               p_plus_minus_bitlength[i], &A24);
        j++;
      }
    }
    ...
    for (int i = 0; i < P_LEN; i++) {
      int j = isog.degree[i];
      while (j < TORSION_ODD_POWERS[i]) {
        xMULv2(&R_plus, &R_plus, &TORSION_ODD_PRIMES[i],
               p_plus_minus_bitlength[i], &A24);
    ...
    for (int i = P_LEN; i < P_LEN + M_LEN; i++) {
      int j = isog.degree[i];
      while (j < TORSION_ODD_POWERS[i]) {
        xMULv2(&R_minus, &R_minus, &TORSION_ODD_PRIMES[i],
               p_plus_minus_bitlength[i], &A24);
