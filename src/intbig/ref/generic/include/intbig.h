 ...
 *
 * Computes quotient, remainder so that remainder+quotient*b = a where
 * 0<=|remainder|<|b| The quotient is rounded towards zero.
 */
 ...
 *
 * Computes quotient, remainder so that remainder+quotient*b = a where
 * 0<=|remainder|<|b| The quotient is rounded towards minus infinity.
 */
void ibz_div_floor(ibz_t *q, ibz_t *r, const ibz_t *n, const ibz_t *d);

/** @brief Euclidean division of a by 2^exp
 *
 * Computes a right shift of abs(a) by exp bits, then sets sign(quotient) to
 * sign(a).
 *
 ...
 *
 * Base should be 10 or 16, and the number should be written without ponctuation
 * or whitespaces
 *
...
/** @brief Copy dig array to target, given digits and the length of the dig
 * array
 *
 ...
 * @param reps Number of Miller-Rabin repetitions. The more, the slower and the
 * less likely are false positives
 * @return 1 if probably prime, 0 if certainly not prime, 2 if certainly prime
 *
 * Using GMP's implementation:
 *
 * From GMP's documentation: "This function performs some trial divisions, a
 * Baillie-PSW probable prime test, then reps-24 Miller-Rabin probabilistic
 * primality tests."
 */
...
/**
 * @brief GCD, Bézout coefficients u, v such that ua + bv = gcd, and
 * annihilators s, t such that sa + bt = 0
 *
 * @param gcd Output: Set to the gcd of a and b
 ...
 * @param b
 */
void ibz_xgcd_ann(ibz_t *gcd, ibz_t *s, ibz_t *t, ibz_t *u, ibz_t *v,
                  const ibz_t *a, const ibz_t *b);

/**
 * @brief Modular inverse
 *
 * @param inv Output: Set to the integer in [0,mod[ such that a*inv = 1 mod
 * (mod) if it exists
 * @param a
 * @param mod
 ...
 * @param mod_b
 */
void ibz_crt(ibz_t *crt, const ibz_t *a, const ibz_t *b, const ibz_t *mod_a,
             const ibz_t *mod_b);
 ...
 *
 * @returns 1 if an integer square root of a exists and was computed, 0
 * otherwise
