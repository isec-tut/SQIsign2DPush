...
bool fp_test() { // Tests for the field arithmetic
  ...
  for (n = 0; n < TEST_LOOPS; n++) {
    fprandom_test(a); fp_tomont(a, a);
    fprandom_test(b); fp_tomont(b, b);
    fprandom_test(c); fp_tomont(c, c);
    fprandom_test(d); fp_tomont(d, d);
  ...
  for (n = 0; n < TEST_LOOPS; n++) {
    ...
    fp_tomont(mc, c);
    fp_mul(md, ma, mb);
    fp_mul(me, md, mc); // e = (a*b)*c
    fp_mul(md, mb, mc);
    fp_mul(mf, md, ma); // f = a*(b*c)
    fp_frommont(e, me);
    fp_frommont(f, mf);
    if (compare_words(e, f, NWORDS_FIELD) != 0) {
      passed = 0;
      break;
    }
    ...
    fp_tomont(mc, c);
    fp_add(md, mb, mc);
    fp_mul(me, ma, md); // e = a*(b+c)
    fp_mul(md, ma, mb);
    fp_mul(mf, ma, mc);
    fp_add(mf, md, mf); // f = a*b+a*c
    fp_frommont(e, me);
    fp_frommont(f, mf);
    if (compare_words(e, f, NWORDS_FIELD) != 0) {
      passed = 0;
    ...
    fp_frommont(d, md);
    fp_frommont(e, me);
    if (compare_words(d, e, NWORDS_FIELD) != 0) {
      passed = 0;
      break;
    }
    ...
    fp_frommont(a, ma);
    fp_frommont(d, md);
    if (compare_words(a, d, NWORDS_FIELD) != 0) {
      passed = 0;
      break;
    }
    ...
    fp_frommont(d, md);
    if (compare_words(b, d, NWORDS_FIELD) != 0) {
      passed = 0;
      break;
    }
  }
  if (passed == 1)
    printf("  GF(p) multiplication tests "
           "...................................... PASSED");
  else {
  ...
  for (n = 0; n < TEST_LOOPS; n++) {
    ...
    fp_frommont(b, mb);
    fp_frommont(c, mc);
    if (compare_words(b, c, NWORDS_FIELD) != 0) {
      passed = 0;
      break;
    }
    ...
    fp_tomont(ma, a);
    fp_sqr(md, ma); // d = 0^2
    if (!fp_is_equal(ma, md)) {
      passed = 0;
      break;
    }
  }
  if (passed == 1)
    printf("  GF(p) squaring "
           "tests............................................. PASSED");
  else {
  ...
  for (n = 0; n < TEST_LOOPS; n++) {
    ...
    fp_frommont(c, mc);
    if (compare_words(c, d, NWORDS_FIELD) != 0) {
      passed = 0;
      break;
    }
  ...
  for (n = 0; n < TEST_LOOPS; n++) {
    ...
    fp_frommont(d, md);
    if ((compare_words(a, c, NWORDS_FIELD) != 0) &&
        (compare_words(a, d, NWORDS_FIELD) != 0)) {
      passed = 0;
      break;
    }
  }
...
bool fp_run() {
  ...
  printf("  GF(p) subtraction runs in ....................................... "
         "%7lld cycles",
         cycles / BENCH_LOOPS);
  printf("\n");
  ...
  printf("  GF(p) multiplication runs in .................................... "
         "%7lld cycles",
         cycles / BENCH_LOOPS);
  printf("\n");
  ...
  printf("  GF(p) square root runs in ....................................... "
         "%7lld cycles",
         cycles / BENCH_LOOPS);
  printf("\n");
