...
int compare_words(
    digit_t *a, digit_t *b,
    unsigned int
        nwords) { // Comparing "nword" elements, a=b? : (1) a>b, (0) a=b, (-1)
                  // a<b SECURITY NOTE: this function does not have
                  // constant-time execution. TO BE USED FOR TESTING ONLY.
  int i;
...
static void sub_test(
    digit_t *out, digit_t *a, digit_t *b,
    unsigned int nwords) { // Subtraction without borrow, out = a-b where a>b
                           // SECURITY NOTE: this function does not have
                           // constant-time execution. It is for TESTING ONLY.
  unsigned int i;
...
void fp2random_test(fp2_t *a) { // Generating a pseudo-random element in GF(p^2)
                                // SECURITY NOTE: distribution is not fully
                                // uniform. TO BE USED FOR TESTING ONLY.
