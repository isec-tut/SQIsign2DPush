...
static uint64_t lindiv31abs(digit_t *d, const digit_t *a, const digit_t *b, uint64_t f, uint64_t g) {
  ...
  (void)inner_fp_sbb(cc, d5 ^ t, t, &d5);
...
bool fp_is_square(const digit_t *x) {
  ...
  for (int i = 0; i < 23; i++) {
    ...
    uint64_t tnza = (a[5] & tnz5) | (a[4] & tnz4) | (a[3] & tnz3) | (a[2] & tnz2) | (a[1] & tnz1);
    uint64_t tnzb = (b[5] & tnz5) | (b[4] & tnz4) | (b[3] & tnz3) | (b[2] & tnz2) | (b[1] & tnz1);
    uint64_t snza = (a[4] & tnz5) | (a[3] & tnz4) | (a[2] & tnz3) | (a[1] & tnz2) | (a[0] & tnz1);
    uint64_t snzb = (b[4] & tnz5) | (b[3] & tnz4) | (b[2] & tnz3) | (b[1] & tnz2) | (b[0] & tnz1);
